...
class User {
  ...
  function User($user_id = 0, $is_logged = FALSE) {
    $this->user_id = $user_id;
    $this->is_logged = $is_logged;
  }

  function hasAccess($realm) {
    if (!$this->user_id || !$this->is_logged) return false;
    $this->getRealms();
    return $this->realms[$realm] == 1;
  }

  function has_restrict($realm) {
    if (!$this->user_id || !$this->is_logged) return false;
    $this->getRealms();
    return $this->realms[$realm] == 1;
  }
  ...
  function link_to_user($nick, $id, $blank = FALSE) {
